...
struct remote_dev {
  ...
  {NULL, 0x1, 0x10/*baseboard*/, {0x01}, 0x1, "baseboard"},
  {NULL, 0x1, 0x05/*2ou exp*/  , {0x07}, 0x1, "1ou expansion card"},
  {NULL, 0x1, 0x15/*1ou exp*/  , {0x07}, 0x1, "2ou expansion card"}
};
...
#define FIELD_LEN(x)        (x & 0x1F)
  ...
  if ( argc == 2 ) {
    slot_id = (uint8_t)strtol(argv[1], NULL, 16);
    specify_id = 1;
    printf("specify the slot id: %d\n", slot_id);
  }
  ...
  for ( i = 0; i < remote_snr_size; i++ ) {
    ...
    //set the path
    snprintf(path, sizeof(path), "/tmp/sdr_slot%d.%d.bin", slot_id, remote_snr_list[i].intf);
    remote_snr_list[i].path = path;
    //init sdr
    if ( sdr_init(remote_snr_list[i].path, &sinfo) < 0 ) {
      printf("---%s is not existed---\n", remote_snr_list[i].desc);
      continue;
    }
