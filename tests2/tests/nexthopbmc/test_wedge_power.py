...
class BaseWedgePowerTest(unittest.TestCase):
    ...
    def wait_for_pltrst_high(self, timeout: int):
        """
        Polls CPU_RST_L until it reads 1 or the timeout elapses.

        Returns:
            bool: True if CPU_RST_L went high within the timeout, else False.
        """
        cmd = "source /usr/local/bin/gpio-utils.sh && gpio_get_value CPU_RST_L"
...
class WedgePowerTest(BaseWedgePowerTest):
    ...
    def test_wedge_power_reset(self):
        ...
        # Wait for CPU_RST_L to go high before triggering the reset.
        if not self.wait_for_pltrst_high(timeout=10):
            Logger.warn("CPU_RST_L did not go high within 10s. But trigger reset anyways")
        self.assertEqual(
