...
static int
get_pxe_crc(int fd, uint8_t addr, uint32_t *crc) {
  int ret = -1;
...
static int
cache_pxe_crc(uint8_t bus, uint8_t addr, char *key, char *checksum) {
  int fd, ret = -1;
  ...
  do {
    ...
    if ((ret = get_pxe_crc(fd, addr, (uint32_t *)rbuf))) {
      break;
    }
...
get_pxe_ver(struct vr_info *info, char *ver_str) {
  ...
  if (kv_get(key, tmp_str, NULL, 0)) {
    if (cache_pxe_crc(info->bus, info->addr, key, tmp_str))
      return -1;
  }
...
static int
program_pxe(uint8_t bus, uint8_t addr, struct pxe_config *config, bool force) {
  int fd, i, ret = -1;
  ...
  do {
    if ((ret = get_pxe_crc(fd, addr, &crc))) {
      break;
    }
...
pxe_fw_update(struct vr_info *info, void *args) {
  ...
  do {
    ...
    ret = program_pxe(info->bus, info->addr, config, info->force);
    if (ret) {
      break;
    }
  } while (0);
...
pxe_fw_verify(struct vr_info *info, void *args) {
  ...
  // check CRC
  ret = get_pxe_crc(fd, info->addr, &crc);
  buf[0] = VR_REG_PAGE;
  buf[1] = 0x00;
