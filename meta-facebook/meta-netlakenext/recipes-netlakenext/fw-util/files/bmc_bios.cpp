...
void BmcBiosComponent::set_ME_Mode(uint8_t mode) {
  ...
  */
  ...
  ret = netlakenext_common_me_ipmb_wrapper(NETFN_NM_REQ, CMD_NM_FORCE_ME_RECOVERY, tbuf, sizeof(tbuf), rbuf_quitMERecovery, &rlen);
  if (ret < 0) {
...
int BmcBiosComponent::getSelftestResult() {
  ...
  */
  ...
  ret = netlakenext_common_me_ipmb_wrapper(NETFN_APP_REQ, CMD_APP_GET_SELFTEST_RESULTS, NULL, 0, rbuf_getSelftestResult, &rlen);
  if (ret < 0) {
