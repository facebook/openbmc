...
class FdtPropertyStrings(FdtProperty):
    ...
    def dts_represent(self, depth=0):
        """Get dts string representation"""
        return INDENT * depth + self.name + ' = "' + '", "'.join(self.strings) + '";'
...
class FdtPropertyWords(FdtProperty):
    ...
    def __init__(self, name, words):
        ...
        for word in words:
            if not 0 <= word <= 4294967295:
                raise Exception(
                    "Invalid word value %d, requires 0 <= number <= 4294967295" % word
                )
        if not len(words):
    ...
    def dtb_represent(self, string_store, pos=0, version=17):
        ...
        if strpos < 0:
            strpos = len(string_store)
            string_store += self.name + "\0"
        blob = pack(">III", FDT_PROP, len(self.words) * 4, strpos) + pack("").join(
            [pack(">I", word) for word in self.words]
        )
        pos += len(blob)
        return (blob, string_store, pos)
...
class FdtPropertyBytes(FdtProperty):
    ...
    def __init__(self, name, bytez):
        ...
        for byte in bytez:
            if not -128 <= byte <= 127:
                raise Exception(
                    "Invalid value for byte %d, requires -128 <= number <= 127" % byte
                )
        if not bytez:
    ...
    def dts_represent(self, depth=0):
        ...
        return (
            ...
            + self.name
            + " = ["
            + " ".join(["%02x" % (byte & int("ffffffff", 16)) for byte in self.bytes])
            + "];"
        )
...
class FdtNode(object):
    ...
    def dts_represent(self, depth=0):
        """Get dts string representation"""
        result = ("\n").join([sub.dts_represent(depth + 1) for sub in self.subdata])
        if len(result) > 0:
            result += "\n"
        return INDENT * depth + self.name + " {\n" + result + INDENT * depth + "};"
    ...
    def __eq__(self, node):
        ...
        cmpnames = set(
            [subnode.get_name() for subnode in node if not isinstance(subnode, FdtNop)]
        )
        if curnames != cmpnames:
            return False
        for subnode in [
            subnode for subnode in self.subdata if not isinstance(subnode, FdtNop)
        ]:
    ...
    def merge(self, node):
        ...
        for subnode in [obj for obj in node if isinstance(obj, (FdtNode, FdtProperty))]:
...
class Fdt(object):
    ...
    def to_dtb(self):
        ...
        if self.reserve_entries is not None:
            for entry in self.reserve_entries:
                blob_reserve_entries += pack(">QQ", entry["address"], entry["size"])
        ...
        if self.prenops is not None:
            blob_dt = (
                pack("").join([nop.dtb_represent("")[0] for nop in self.prenops])
                + blob_dt
            )
