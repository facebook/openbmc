...
# get the MAC from the BMC's EEPROM, on FBOSS it is found at Extended MAC Base
# running "weutil_prefd bmc" on a downstream openbmc image yields the same MAC for the Local and Extended MAC base
mac=$(weutil_prefdl bmc | grep '^Extended MAC Base' | cut -d' ' -f4)
