...
def _packet_feutil_date(cmd):
    ...
    data = data.split("\r\n")
    return {"Information": _parse_feutil_data(data), "Actions": [], "Resources": []}
...
def get_feutil_fcm_t_data():
    cmd = ["/usr/local/bin/feutil", "fcm-t"]
    return _packet_feutil_date(cmd)


def get_feutil_fcm_b_data():
    cmd = ["/usr/local/bin/feutil", "fcm-b"]
    return _packet_feutil_date(cmd)
...
def get_feutil_fan5_data():
    cmd = ["/usr/local/bin/feutil", "5"]
    return _packet_feutil_date(cmd)


def get_feutil_fan6_data():
    cmd = ["/usr/local/bin/feutil", "6"]
    return _packet_feutil_date(cmd)


def get_feutil_fan7_data():
    cmd = ["/usr/local/bin/feutil", "7"]
    return _packet_feutil_date(cmd)


def get_feutil_fan8_data():
    cmd = ["/usr/local/bin/feutil", "8"]
    return _packet_feutil_date(cmd)
