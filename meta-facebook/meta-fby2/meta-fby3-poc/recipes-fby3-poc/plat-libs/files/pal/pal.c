...
read_device(const char *device, int *value) {
  ...
  fp = fopen(device, "r");
  if (!fp) {
    syslog(LOG_INFO, "failed to open device %s. errnor:%d msg:%s", device, errno, strerror(errno));
    return -1;
  }
...
power_on_server_physically(uint8_t slot_id){
  ...
  syslog(LOG_WARNING, "%s is not ready for slot%d\n",__func__,slot_id);

  sprintf(vpath, GPIO_VAL, gpio_power_en[slot_id]);
  ...
  switch(fru) {
    ...
    case FRU_NIC:
      ...
      *cnt = nic_sensor_cnt;
      break;
    default:
      syslog(LOG_WARNING, "%s: wrong fru id %u", __func__, fru);
      return -1;
  }
...
pal_get_fan_name(uint8_t num, char *name) {
  ...
  switch(num) {
    ...
    default:
      syslog(LOG_WARNING,"%s: invalid fan num - %d", __func__, num);
      return -1;
  }
...
pal_get_board_id(uint8_t slot, uint8_t *req_data, uint8_t req_len, uint8_t *res_data, uint8_t *res_len)
{
  ...
  sprintf(path, GPIO_VAL, GPIOG1_BOARD_REV_ID1);
  if (read_device(path, &BOARD_REV_ID1)) {
    *res_len = 0;
    return completion_code;
  }

  sprintf(path, GPIO_VAL, GPIOG2_BOARD_REV_ID2);
    ...
    case FRU_SPB:
      switch (snr_num) {
        ...
        case SP_SENSOR_FAN4_TACH:
          sprintf(crisel, "Fan4 %s %.0fRPM - ASSERT", thresh_name, val);
          break;
        case SP_SENSOR_FAN5_TACH:
          sprintf(crisel, "Fan5 %s %.0fRPM - ASSERT", thresh_name, val);
          break;
        case SP_SENSOR_FAN6_TACH:
          sprintf(crisel, "Fan6 %s %.0fRPM - ASSERT", thresh_name, val);
          break;
        case SP_SENSOR_FAN7_TACH:
          sprintf(crisel, "Fan7 %s %.0fRPM - ASSERT", thresh_name, val);
          break;
    ...
    case FRU_SPB:
      switch (snr_num) {
        ...
        case SP_SENSOR_FAN1_TACH:
          ...
          break;
        case SP_SENSOR_FAN2_TACH:
          sprintf(crisel, "Fan2 %s %.0fRPM - DEASSERT", thresh_name, val);
          break;
        case SP_SENSOR_FAN3_TACH:
          sprintf(crisel, "Fan3 %s %.0fRPM - DEASSERT", thresh_name, val);
          break;
        case SP_SENSOR_FAN4_TACH:
          sprintf(crisel, "Fan4 %s %.0fRPM - DEASSERT", thresh_name, val);
          break;
        case SP_SENSOR_FAN5_TACH:
          sprintf(crisel, "Fan5 %s %.0fRPM - DEASSERT", thresh_name, val);
          break;
        case SP_SENSOR_FAN6_TACH:
          sprintf(crisel, "Fan6 %s %.0fRPM - DEASSERT", thresh_name, val);
          break;
        case SP_SENSOR_FAN7_TACH:
          sprintf(crisel, "Fan7 %s %.0fRPM - DEASSERT", thresh_name, val);
          break;
        case SP_SENSOR_P1V15_BMC_STBY:
...
bool pal_is_m2_prsnt(char *slot_name, char *sensor_name)
{
  ...
  syslog(LOG_WARNING, "%s is not ready for fby3-poc", __func__);
  return false;
}
