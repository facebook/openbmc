...
se_fru = CDLL("libwedge_eeprom.so")

class SE_FRU(Structure):
...
def get_seutil():
	sys.stdout.flush()
	card_val=os.popen("i2cget -f -y 12 0x31 0x3 2> /dev/null").read()
	sys.stdout.flush()
	right=os.popen("i2cget -f -y 12 0x31 0x5 2> /dev/null").read()
	if int(card_val, 16) < 8 and int(right, 16) == 1:
		fruinfo = "Right of LC card not the EEPROM"
	else:
	    myfru = SE_FRU()
	    p_myfru = pointer(myfru)
	    se_fru.wedge_eeprom_parse("/sys/class/i2c-adapter/i2c-0/0-0052/eeprom", p_myfru)
