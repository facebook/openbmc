...
#include "fby35_fruid.h"

/* Populate char path[] with the path to the fru's fruid binary dump */
int
fby35_get_fruid_path(uint8_t fru, uint8_t dev_id, char *path) {
  char fname[16] = {0};
  ...
  sprintf(path, FBY35_FRU_DEV_PATH, fname, dev_id);
  return 0;
}

/* Populate char name[] with the path to the fru's name */
int
fby35_get_fruid_name(uint8_t fru, char *name) {
