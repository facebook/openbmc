 ...
 *       uint8_t* buf)
 *   {
 *       return i2c_smbus_read_block_data(handle, cmdCode, buf);
 *   }
 *
 * @param[in]  handle     Handle to I2C driver
 * @param[in]  cmdCode    8-bit command code
 ...
 *                        be placed
 * @param[out] numBytes   Number of bytes to be read
 * @return     int - Error code
 */
 ...
 * @return     AriesErrorType - Aries error code
 */
AriesErrorType ariesSMBusReadBlockData(AriesI2CDriverType* i2cDriver,
                                       uint32_t address, uint8_t numBytes,
                                       uint8_t* values);

/**
 * @brief Write/read multiple data bytes to Aries over I2C  This function
