...
file_get_sel_hdr(int node) {
  ...
  if (fread(&g_sel_hdr[node], sizeof(sel_hdr_t), 1, fp) <= 0) {
    OBMC_WARN("file_get_sel_hdr: fread\n");
    fclose (fp);
    return -1;
  }
...
file_get_sel_data(int node) {
  ...
  fp = fopen(fpath, "r");
  if (fp == NULL) {
    OBMC_WARN("file_get_sel_data: fopen\n");
    return -1;
  }

  if (fseek(fp, SEL_DATA_OFFSET, SEEK_SET)) {
    OBMC_WARN("file_get_sel_data: fseek\n");
    fclose(fp);
    return -1;
  }
  ...
  if (fread(buf, 1, SEL_ELEMS_MAX * sizeof(sel_msg_t), fp) <= 0) {
    OBMC_WARN("file_get_sel_data: fread\n");
    fclose(fp);
    return -1;
  }
...
file_store_sel_hdr(int node) {
  ...
  fp = fopen(fpath, "r+");
  if (fp == NULL) {
    OBMC_WARN("file_store_sel_hdr: fopen\n");
    return -1;
  }

  if (fwrite(&g_sel_hdr[node], sizeof(sel_hdr_t), 1, fp) <= 0) {
    OBMC_WARN("file_store_sel_hdr: fwrite\n");
    fclose(fp);
    return -1;
  }
...
file_store_sel_data(int node, int recId, sel_msg_t *data) {
  ...
  fp = fopen(fpath, "r+");
  if (fp == NULL) {
    OBMC_WARN("file_store_sel_data: fopen\n");
    return -1;
  }
  ...
  if (fseek(fp, SEL_DATA_OFFSET+index, SEEK_SET)) {
    OBMC_WARN("file_store_sel_data: fseek\n");
    fclose(fp);
    return -1;
  }

  if (fwrite(data->msg, sizeof(sel_msg_t), 1, fp) <= 0) {
    OBMC_WARN("file_store_sel_data: fwrite\n");
    fclose(fp);
    return -1;
  }
...
dump_sel_syslog(int fru, sel_msg_t *data) {
  ...
  OBMC_WARN("SEL Entry, FRU: %d, Content: %s\n", fru, str);
}
...
parse_ras_sel(uint8_t fru, ras_sel_msg_t *data) {
  ...
  OBMC_CRIT("SEL Entry: FRU: %d, MFG ID: %s, "
         "%s",
...
parse_sel(uint8_t fru, sel_msg_t *data) {
  ...
  if (record_type < 0xC0) {
    ...
    OBMC_CRIT("SEL Entry: FRU: %d, Record: %s (0x%02X), Time: %s, "
    ...
    if ( record_type == 0xFB )
    {
      ...
      OBMC_CRIT("SEL Entry: FRU: %d, Record: %s (0x%02X), %s", fru, error_type, record_type, error_log); 
    }
    else
    {
      ...
      OBMC_CRIT("SEL Entry: FRU: %d, Record: %s (0x%02X), "
          "OEM Data: (%s) %s ",
          fru,
          error_type, record_type,
          oem_data, error_log);
...
sel_get_entry(int node, int read_rec_id, sel_msg_t *msg, int *next_rec_id) {
  ...
  if (sel_num_entries(node) == 0) {
    OBMC_WARN("sel_get_entry: No entries\n");
    return -1;
  }

  // Check for boundary conditions
  if ((index < SEL_INDEX_MIN) || (index > SEL_INDEX_MAX)) {
    OBMC_WARN("sel_get_entry: Invalid Record ID %d\n", read_rec_id);
    return -1;
  }
  ...
  if (g_sel_hdr[node].begin < g_sel_hdr[node].end) {
    if (index < g_sel_hdr[node].begin || index >= g_sel_hdr[node].end) {
      OBMC_WARN("sel_get_entry: Wrong Record ID %d\n", read_rec_id);
      return -1;
    }
  }
  ...
  if (g_sel_hdr[node].begin > g_sel_hdr[node].end) {
    if (index >= g_sel_hdr[node].end && index < g_sel_hdr[node].begin) {
      OBMC_WARN("sel_get_entry: Wrong Record ID2 %d\n", read_rec_id);
      return -1;
    }
  }
...
sel_add_entry(int node, sel_msg_t *msg, int *rec_id) {
  ...
  if (sel_num_entries(node) == SEL_RECORDS_MAX) {
      OBMC_WARN("sel_add_entry: SEL rollover\n");
  ...
  *rec_id = g_sel_hdr[node].end+1;
  ...
  if (file_store_sel_data(node, *rec_id, msg)) {
    OBMC_WARN("sel_add_entry: file_store_sel_data\n");
    return -1;
  }
  ...
  if (file_store_sel_hdr(node)) {
    OBMC_WARN("sel_add_entry: file_store_sel_hdr\n");
    return -1;
  }
...
sel_erase(int node, int rsv_id) {
  ...
  // Store the structure persistently
  if (file_store_sel_hdr(node)) {
    OBMC_WARN("sel_erase: file_store_sel_hdr\n");
    return -1;
  }
...
sel_node_init(int node) {
  ...
  if (access(fpath, F_OK) == 0) {
    ...
    if (file_get_sel_hdr(node)) {
      OBMC_WARN("init_sel: file_get_sel_hdr\n");
      return -1;
    }

    if (file_get_sel_data(node)) {
      OBMC_WARN("init_sel: file_get_sel_data\n");
      return -1;
    }
  ...
  if (file_store_sel_hdr(node)) {
    OBMC_WARN("init_sel: file_store_sel_hdr\n");
    return -1;
  }
  ...
  for (i = 1;  i <= SEL_RECORDS_MAX; i++) {
    ...
    if (file_store_sel_data(node, i, &msg)) {
      OBMC_WARN("init_sel: file_store_sel_data\n");
      return -1;
    }
  }
