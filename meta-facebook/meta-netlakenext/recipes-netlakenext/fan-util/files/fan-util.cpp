...
sensor_fail_check(bool status) {
  ...
  if (access(SENSOR_FAIL_FILE, F_OK) == 0) {
    ...
    if (dir != NULL) {
      int cnt = 0;
      cout << "Sensor Fail: ";
      for (const struct dirent *directoryEntry = readdir(dir);
           directoryEntry != NULL; directoryEntry = readdir(dir)) {
...
fan_mode_check(bool printMode) {
  ...
  if (printMode) {
    uint8_t mode;
    int fd = open(FAN_MODE_FILE, O_RDONLY);
    if (fd < 0) {
      cout << "Fan Mode: Unknown" << endl;
      return false;
    }

    int cnt = read(fd, &mode, sizeof(uint8_t));
...
main(int argc, char **argv) {
  ...
  PWM_INFO *pwm_info =
      static_cast<PWM_INFO *>(malloc(MAX_ZONE_NUM * sizeof(PWM_INFO)));
  ...
  if(argc > 1) {
    if (strcmp(argv[1], "--set") == 0)
      ...
      strcpy(argv[1], "get");
    else if (strcmp(argv[1], "--set-kv") == 0)
      strcpy(argv[1], "set-kv");
    else if (strcmp(argv[1], "--get-kv") == 0)
      strcpy(argv[1], "get-kv");
  ...
  auto setkvCommand = app.add_subcommand("set-kv", "set the kv value of pwm");
  setkvCommand->add_option("kv_value", kv_value, "%% kv_value to set")->check(CLI::Range(0,100))->required();
  ...
  auto getkvCommand = app.add_subcommand("get-kv", "get the kv value of fan_rpm_cfg");
  ...
  } else if (setkvCommand->parsed()) {
    ret = pal_set_pwm_kv_value(kv_value);
    if (ret < 0) {
      cout << "Error while setting fan_rpm_cfg kv value: " << kv_value << endl;
      goto error_exit;
    }
  } else if (getkvCommand->parsed()) {
    if (pal_get_pwm_kv_value(&kv_value) == 0) {
      cout << "fan_rpm_cfg kv value: " << kv_value << endl;
    } else {
       cout << "Error while getting fan_rpm_cfg kv value" << endl;
       goto error_exit;
    }
  }
