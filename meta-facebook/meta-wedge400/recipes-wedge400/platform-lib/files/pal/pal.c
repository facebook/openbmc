...
scm_sensor_read(uint8_t sensor_num, float *value) {
  ...
  if (scm_sensor) {
    ...
    if (!g_sinfo[FRU_SCM-1][sensor_num].valid) {
      bic_sdr_init(FRU_SCM, true); //reinit g_sinfo
      if (!g_sinfo[FRU_SCM-1][sensor_num].valid) {
        return READING_NA;
      }
    }
  ...
  switch (fru) {
    ...
    case FRU_PSU2:
      ...
      if (is_psu48())
      {
        ...
        psu_sensor_threshold[PSU1_SENSOR_FAN2_TACH + (fru_offset * PSU1_SENSOR_CNT)][UCR_THRESH] = 29700; // Used to be 21950, change to 27000 * 110% = 29700.
