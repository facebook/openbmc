...
clear_crashdump_archive()
{
  ...
  for i in /mnt/data/crashdump_server*.tar.gz
  do
...
dump_sensor_history() 
{
  ...
  bmc=$(/usr/local/bin/sensor-util bmc --history $l_SENSOR_HISTORY) && echo "$bmc"
  pdb=$(/usr/local/bin/sensor-util pdb --history $l_SENSOR_HISTORY) && echo "$pdb"
  fio=$(/usr/local/bin/sensor-util fio --history $l_SENSOR_HISTORY) && echo "$fio"
...
dump_sensor_threshold()
{
  ...
  bmc=$(/usr/local/bin/sensor-util bmc --threshold) && echo "$bmc"
  pdb=$(/usr/local/bin/sensor-util pdb --threshold) && echo "$pdb"
  fio=$(/usr/local/bin/sensor-util fio --threshold) && echo "$fio"
...
for i in "$@"; do
  case "$1" in
    ...
    --*|-*=) # unsupported flags
      ...
      exit 1
      ;;
    slot1) # preserve positional arguments
      SLOT_NAME="server"
...
case $SLOT_NAME in
    server)
      ...
      ;;
    *)
      echo "Usage: $(script_filename) {slot1}"
      exit 1
      ;;
esac
...
CRASHDUMP_FILE="/tmp/crashdump_server"

CRASHDUMP_LOG_ARCHIVE="/mnt/data/crashdump_server_${CURT_DTIME}.tar.gz"
LOG_MSG_PREFIX=""
