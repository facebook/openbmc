 ...
 */
typedef struct {
  NCSI_Meta_Get_Debug_Data_Request req;
  FILE      *fp;
  uint16_t  type;
  uint32_t  crc;
  uint32_t  total_len;
...
static int get_debug_data_name_to_type(const char *name)
{
  if (!strcasecmp(name, "coredump"))
    return NCSI_META_GET_DEBUG_DATA_TYPE_CORE_DUMP;
  else if (!strcasecmp(name, "crashdump"))
    return NCSI_META_GET_DEBUG_DATA_TYPE_CRASH_DUMP;
  else
    return -1;
}
                                    ...
                                    uint16_t type)
{
  ...
  gdd_txn->req.op = NCSI_META_GET_DEBUG_DATA_REQ_OP_START_XFER;
}
...
{
  if (ncsi_meta_create_req_pkt(nl_msg, util_args->channel_id,
                               NCSI_META_GET_DEBUG_DATA,
                                      ...
                                      bool *txn_completed)
{
  ...
  /* Check Meta OEM response */
  if (ncsi_meta_check_resp(nl_resp, NCSI_META_GET_DEBUG_DATA,
                           sizeof(*gdd_resp))) {
  ...
  is_first = (gdd_txn->req.op == NCSI_META_GET_DEBUG_DATA_REQ_OP_START_XFER);
  if (ncsi_meta_check_get_debug_data(nl_resp, gdd_txn->type, is_first)) {
  ...
  if (gdd_resp->op & NCSI_META_GET_DEBUG_DATA_RESP_OP_START) {
    ...
    gdd_txn->req.op = NCSI_META_GET_DEBUG_DATA_REQ_OP_GET_NEXT;
  }
  ...
  if (data_len > 0) {
    ...
    if (n < data_len) {
      printf("Error: failed to write %zu data bytes to file, rv %zu\n",
             data_len, n);
      return -1;
    }
  }
                          ...
                          ncsi_util_args_t *util_args)
{
  ...
  if (!nl_msg) {
    printf("Error: failed to allocate nl_msg of %zu bytes\n", sizeof(*nl_msg));
    goto done;
  }
...
static void meta_ncsi_util_usage(void)
{
  printf("Usage: ncsi-util -m meta [options]\n\n");
  printf("       -h             This help\n");
...
int meta_ncsi_util_handler(int argc, char **argv, ncsi_util_args_t *util_args)
{
