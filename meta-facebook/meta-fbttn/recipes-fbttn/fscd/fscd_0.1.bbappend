...
SRC_URI += "file://init_pwm.sh \
            file://setup-fan.sh \
            file://FSC_BC_EVT_v1_config.json \
...
do_install() {
  ...
  install -m 644 FSC_BC_EVT_v1_config.json ${D}${sysconfdir}/fsc-config.json
  install -m 644 FSC_BC_EVT_v1_zone0.fsc ${D}${sysconfdir}/fsc/FSC_BC_EVT_v1_zone0.fsc
  install -m 644 FSC_BC_EVT_v1_zone1.fsc ${D}${sysconfdir}/fsc/FSC_BC_EVT_v1_zone1.fsc
  install -m 755 setup-fan.sh ${D}${sysconfdir}/init.d/setup-fan.sh
  install -m 755 run-fscd.sh ${D}${sysconfdir}/sv/fscd/run
  update-rc.d -r ${D} setup-fan.sh start 98 5 .
}
