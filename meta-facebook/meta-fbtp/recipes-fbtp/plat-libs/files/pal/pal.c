...
static int
pal_update_MCU_bl_fw(uint8_t target, uint32_t offset, uint16_t len, uint8_t *buf)
{
  uint8_t tbuf[256] = {0x00};
  uint8_t rbuf[256] = {0x00};
  ipmb_req_t *req;
...
pal_update_MCU_bl(char *path)
{
  ...
  while (1) {
    ...
    ret = pal_update_MCU_bl_fw(target, offset, count, buf);
    if (ret == 0) {
