...
def usage():
    print(
        "Usage: bios-util [ boot_order, plat_info, pcie_config, pcie_port_config, postcode ] <options>"
    )
    exit(-1)
...
def usage_boot_order():
    ...
    print(
        "                            --force_boot_BIOS_setup <--enable, --disable, --get>"
    )
    print(
        "                            --boot_order [ --set < #1st #2nd #3rd #4th #5th >, --get, --disable ]"
    )
...
def usage_PCIE_port_config():
    ...
    if iom_type == IOM_M2:
        print(
            "Usage: bios-util pcie_port_config [ --set <--enable, --disable> <scc_ioc, flash1, flash2, nic>, --get ]"
        )
    elif iom_type == IOM_IOC:
        print(
            "Usage: bios-util pcie_port_config [ --set <--enable, --disable> <scc_ioc, iom_ioc, nic>, --get ]"
        )
    exit(-1)
...
def boot_order(argv):
    ...
    req_data[0] = (
        ((data[0] & ~0x86) | (clear_CMOS << 1)) | (force_boot_BIOS_setup << 2)
    ) | (boot_flags_valid << 7)
