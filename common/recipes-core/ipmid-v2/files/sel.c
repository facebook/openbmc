...
static int file_get_sel_hdr(int node) {
  FILE* fp;
...
static int file_get_sel_data(int node) {
  FILE* fp;
  int i, j;
  char fpath[SIZE_PATH_MAX] = {0};
...
static int file_store_sel_hdr(int node) {
  FILE* fp;
  char fpath[SIZE_PATH_MAX] = {0};
...
static int file_store_sel_data(int node, int recId, sel_msg_t* data) {
  FILE* fp;
  int index;
  char fpath[SIZE_PATH_MAX] = {0};
...
static void dump_sel_syslog(int fru, sel_msg_t* data) {
  int i = 0;
  char temp_str[8] = {0};
  char str[128] = {0};
...
static void dump_ras_sel_syslog(uint8_t fru, ras_sel_msg_t* data) {
  int i = 0;
  char temp_str[8] = {0};
  char str[256] = {0};
...
static void parse_ras_sel(uint8_t fru, ras_sel_msg_t* data) {
...
static void parse_sel(uint8_t fru, sel_msg_t* data) {
  uint32_t timestamp;
  uint8_t* sel = data->msg;
  uint8_t sensor_num;
  uint8_t record_type;
  char sensor_name[32];
  char error_log[256] = {0};
  char error_type[64];
  char oem_data[32];
  int ret;
  struct tm ts;
  ...
  **/
  ...
  if (record_type < 0xC0) {
    ...
    ipmi_get_event_sensor_name(fru, sel, sensor_name);

    /* Event Data (Byte 13:15) */
    ret = ipmi_parse_sel_helper(fru, sel, error_log);
    sprintf(event_data, "%02X%02X%02X", sel[13], sel[14], sel[15]);

    /* Check if action needs to be taken based on the SEL message */
    if (!ret)
      ret = ipmi_sel_handler(fru, sensor_num, &sel[10]);

    syslog(
        LOG_CRIT,
        "SEL Entry: FRU: %d, Record: %s (0x%02X), Time: %s, "
    ...
    ipmi_parse_oem_sel(fru, sel, error_log);
    ...
    if (record_type == 0xFB) {
      ...
      ipmi_parse_oem_unified_sel(fru, sel, error_log);
      syslog(
          LOG_CRIT,
          "SEL Entry: FRU: %d, Record: %s (0x%02X), %s",
          fru,
          error_type,
          record_type,
          error_log);
      ...
      syslog(
          LOG_CRIT,
          "SEL Entry: FRU: %d, Record: %s (0x%02X), "
          "OEM Data: (%s) %s ",
          fru,
          error_type,
          record_type,
...
static int sel_node_init(int node) {
  FILE* fp;
  int i;
  char fpath[SIZE_PATH_MAX] = {0};
