...
#define IPMI_PTHREAD_MUTEX_INIT(_mx)                         \
  do {                                                       \
    if (pthread_mutex_init(&_mx, NULL)) {                    \
      syslog(                                                \
          LOG_ERR,                                           \
          "failed intializing pthread_mutex! error: %s(%d)", \
          strerror(errno),                                   \
          errno);                                            \
      free_cfg_items();                                      \
      exit(1);                                               \
...
#define IPMI_PTHREAD_MUTEX_DESTROY(_mx)                  \
  do {                                                   \
    if (pthread_mutex_destroy(&_mx)) {                   \
      syslog(                                            \
          LOG_ERR,                                       \
          "failed destroy pthread_mutex! error: %s(%d)", \
          strerror(errno),                               \
          errno);                                        \
    };                                                   \
  } while (0)
...
plat_cfg_t _cfg = {0};
ipmi_helper_funcs_t _func_vector = {0};

plat_cfg_t* g_cfg = &_cfg;
ipmi_helper_funcs_t* g_fv = &_func_vector;
...
static char* cpu_info_key[] =
    {"", "product_name", "basic_info", "type", "micro_code", "turbo_mode"};
...
// Since the global data is specific to a NetFunction, adding locs at NetFn
// level
...
extern int plat_udbg_get_post_desc(
    uint8_t index,
    uint8_t* next,
    uint8_t phase,
    uint8_t* end,
    uint8_t* length,
    uint8_t* buffer);
extern int plat_udbg_get_gpio_desc(
    uint8_t index,
    uint8_t* next,
    uint8_t* level,
    uint8_t* def,
    uint8_t* count,
    uint8_t* buffer);
extern int plat_udbg_get_frame_data(
    uint8_t frame,
    uint8_t page,
    uint8_t* next,
    uint8_t* count,
    uint8_t* buffer);
extern int plat_udbg_control_panel(
    uint8_t panel,
    uint8_t operation,
    uint8_t item,
    uint8_t* count,
...
//
static void ipmi_handle(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len);
static void ipmi_handle_oem_1s(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len);
static void ipmi_handle_oem_zion(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len);
...
void* clear_bios_data_timer(void* ptr) {
  ...
  ipmi_get_boot_order(slot_id, NULL, boot, &res_len);
  pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, &oldstate);

#ifdef DEBUG
  syslog(
      LOG_WARNING,
      "[%s][%lu] Get: %x %x %x %x %x %x\n",
      __func__,
      pthread_self(),
      boot[0],
      boot[1],
  ...
  // set data
  pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &oldstate);
  ipmi_set_boot_order(slot_id, boot, NULL, &res_len);
  pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, &oldstate);
...
static void chassis_get_status(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  res->cc = CC_SUCCESS;
  *res_len = 0;
  ipmi_get_chassis_status(req->payload_id, req->data, res->data, res_len);
}
...
static void chassis_control(
  ...
  ret = ipmi_chassis_control(req->payload_id, req->data, (req_len - 3));
  res->cc = ret;
  *res_len = 0;
  return;
}
// Set Power Restore Policy (IPMI/Section 28.8)
static void chassis_set_power_restore_policy(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  *data++ = 0x07; // Power restore policy support(bitfield)
  ...
  res->cc =
      ipmi_set_power_restore_policy(req->payload_id, req->data, res->data);
...
static void chassis_get_system_restart_cause(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  *res_len = 0;
  res->cc = CC_SUCCESS;
  if (get_restart_cause(req->payload_id, &data[0])) {
...
static void chassis_get_boot_options(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void chassis_set_boot_options(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void ipmi_handle_chassis(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void sensor_plat_event_msg(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  if (req_len == 11) { // For messaging from system interface
    ...
    memcpy(
        &entry.msg[9],
        req->data,
        7); // Platform event provides only last 7 bytes of SEL's 16-byte entry
  }
...
static void sensor_alert_immediate_msg(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void sensor_set_reading(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void ipmi_handle_sensor(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void app_cold_reset(
  ...
  *res_len = 0;
  res->cc = CC_SUCCESS;
  if (g_fv->is_fw_update_ongoing_system()) {
    res->cc = CC_NODE_BUSY;
    return;
  }
  for (i = 1; i < MAX_NUM_FRUS; i++) {
    if (ipmi_is_crashdump_ongoing(i)) {
      res->cc = CC_NODE_BUSY;
      return;
    }
  }
...
static void app_get_selftest_results(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void app_manufacturing_test_on(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  if ((!memcmp(req->data, "sled-cycle", strlen("sled-cycle"))) &&
    ...
    if (system("/usr/local/bin/power-util sled-cycle") != 0) {
      ...
      (!memcmp(req->data, "obmc-dump", strlen("obmc-dump"))) &&
    ...
    if ((is_first_exc == true) ||
        strncmp(stat_buf, "Ongoing", strlen("Ongoing")) != 0) {
      if (system("/usr/local/bin/obmc-dump -y &") != 0) {
        res->cc = CC_NOT_SUPP_IN_CURR_STATE;
      }
    }
  } else if (
      (!memcmp(req->data, "obmc-dump -s", strlen("obmc-dump -s"))) &&
...
static void app_get_device_guid(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  ret = g_fv->get_plat_guid(req->payload_id, (char*)res->data);
  if (ret) {
    res->cc = CC_UNSPECIFIED_ERROR;
    *res_len = 0x00;
  } else {
...
static void app_get_device_sys_guid(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_set_device_sys_guid(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  // Get the 16 bytes of System GUID from PAL library
  ret = g_fv->set_plat_guid(req->payload_id, (char*)req->data);
  if (ret) {
    res->cc = CC_UNSPECIFIED_ERROR;
    *res_len = 0x00;
  } else {
...
static void app_reset_watchdog_timer(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void app_set_watchdog_timer(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void app_get_watchdog_timer(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void app_set_global_enables(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void app_get_global_enables(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void app_clear_message_flags(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void app_set_sys_info_params(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  switch (param) {
    ...
    case SYS_INFO_PARAM_SYSFW_VER:
      ...
      ipmi_set_sysfw_ver(req->payload_id, g_sys_info_params.sysfw_ver);
      break;
    ...
    case SYS_INFO_PARAM_BIOS_CURRENT_BOOT_LIST:
      memcpy(
          g_sys_info_params.bios_current_boot_list,
          &req->data[1],
          req_len - 4); // boot list length = req_len-4 (payload_id, cmd, netfn,
                        // param)
      break;
    case SYS_INFO_PARAM_BIOS_FIXED_BOOT_DEVICE:
      if (length_check(
              SIZE_BIOS_FIXED_BOOT_DEVICE + 1, req_len, response, res_len))
        break;
      memcpy(
          g_sys_info_params.bios_fixed_boot_device,
          &req->data[1],
          SIZE_BIOS_FIXED_BOOT_DEVICE);
      break;
    case SYS_INFO_PARAM_BIOS_RESTORES_DEFAULT_SETTING:
      if (length_check(
              SIZE_BIOS_RESTORES_DEFAULT_SETTING + 1,
              req_len,
              response,
              res_len))
        break;
      memcpy(
          g_sys_info_params.bios_restores_default_setting,
          &req->data[1],
          SIZE_BIOS_RESTORES_DEFAULT_SETTING);
      break;
    case SYS_INFO_PARAM_LAST_BOOT_TIME:
      if (length_check(SIZE_LAST_BOOT_TIME + 1, req_len, response, res_len))
        break;
      memcpy(
          g_sys_info_params.last_boot_time, &req->data[1], SIZE_LAST_BOOT_TIME);
      break;
    default:
      res->cc = CC_INVALID_PARAM;
      break;
...
static void app_get_sys_info_params(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
    ...
    switch (param) {
      case SYS_INFO_PARAM_SET_IN_PROG:
        *data++ = g_sys_info_params.set_in_prog;
        break;
      case SYS_INFO_PARAM_SYSFW_VER:
        ipmi_get_sysfw_ver(req->payload_id, g_sys_info_params.sysfw_ver);
...
static void app_master_write_read(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  if (ptr[0] & 0x01) {
    if ((readCnt > 35) || (writeCnt > 36)) {
      ...
      // ret = pal_i2c_write_read(bus_num, req->data[1], buf, writeCnt,
      // res->data, readCnt);
      ...
      if (!ret) {
        *res_len = readCnt;
        ...
        syslog(
            LOG_WARNING,
            "app_master_write_read: master read fail, bus %d, addr %x",
            bus_num,
            req->data[1]);
        res->cc = CC_UNSPECIFIED_ERROR;
      }
...
static void app_get_sys_intf_caps(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void storage_get_sel(
  ...
  if (len == 0xFF) { // FFh means read entire record
    ...
    len = SIZE_SEL_REC;
      ...
      (offset >= SIZE_SEL_REC) || (len > SIZE_SEL_REC) ||
      ((offset + len) > SIZE_SEL_REC)) {
    res->cc = CC_PARAM_OUT_OF_RANGE;
    return;
  }
...
static void storage_get_sel_utc(
    unsigned char* response,
    unsigned char* res_len) {
  ipmi_res_t* res = (ipmi_res_t*)response;
  unsigned char* data = &res->data[0];
...
static void ipmi_handle_storage(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void transport_set_lan_config(
    unsigned char* request,
    unsigned char* response,
    unsigned char* res_len) {
  ipmi_mn_req_t* req = (ipmi_mn_req_t*)request;
...
static void transport_get_lan_config(
    unsigned char* request,
    unsigned char* response,
    unsigned char* res_len) {
  ipmi_mn_req_t* req = (ipmi_mn_req_t*)request;
...
static void transport_get_sol_config(
    unsigned char* request,
    unsigned char* response,
    unsigned char* res_len) {
  ipmi_mn_req_t* req = (ipmi_mn_req_t*)request;
...
static void ipmi_handle_transport(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_q_sled_cycle_prepare_request(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_q_sled_cycle_prepare_status(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
 ...
 */
static void oem_q_set_proc_info(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  sprintf(
      key,
      "sys_config/fru%d_cpu%d_%s",
      req->payload_id,
      req->data[3],
      cpu_info_key[req->data[4]]);
...
static void oem_q_get_proc_info(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_q_set_dimm_info(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  sprintf(
      key,
      "sys_config/fru%d_dimm%d_%s",
      req->payload_id,
      req->data[3],
      dimm_info_key[req->data[4]]);
...
static void oem_q_get_dimm_info(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_q_set_drive_info(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  sprintf(
      key,
      "sys_config/fru%d_%c_drive%d_%s",
      req->payload_id,
      cltrType,
      req->data[4],
      drive_info_key[req->data[5]]);
...
static void oem_q_get_drive_info(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_q_set_smu_psp_ver(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_q_get_smu_psp_ver(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_set_boot_order(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  if (req->data[0] &
      (BIOS_BOOT_VALID_FLAG | CMOS_VALID_FLAG |
       FORCE_BOOT_BIOS_SETUP_VALID_FLAG)) {
    /*Create timer thread*/
    ret = pthread_create(
  ...
  ret = ipmi_set_boot_order(req->payload_id, req->data, res->data, res_len);
...
static void oem_get_boot_order(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  ret = ipmi_get_boot_order(req->payload_id, req->data, res->data, res_len);
...
static void oem_set_tpm_presence(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_get_tpm_presence(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  *res_len = 0;
  ...
  switch (selector) {
    case PPR_ACTION:
      ...
      if (ppr_enable) {
        if (ppr_type != PPR_SOFT && ppr_type != PPR_HARD &&
            ppr_type != PPR_TEST_MODE) {
          res->cc = CC_INVALID_DATA_FIELD;
          return;
        }
    ...
    case PPR_HISTORY_DATA:
      ...
      sprintf(
          filepath, "/mnt/data/ppr/fru%d_ppr_history_data", req->payload_id);
      fp = fopen(filepath, "a+");
      fseek(fp, 0, SEEK_END);
      size = ftell(fp);
      fseek(fp, 0, SEEK_SET);
...
static void oem_set_post_start(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_set_ppin_info(
  ...
  res->cc = ipmi_set_ppin_info(
      req->payload_id, req->data, req_len, res->data, res_len);
  return;
}

static void oem_set_adr_trigger(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_get_plat_info(
  ...
  ret = g_fv->is_fru_prsnt(req->payload_id, &pres);
  if (ret) {
    res->cc = CC_UNSPECIFIED_ERROR;
    *res_len = 0x00;
    return;
  ...
  if (g_fv->is_test_board()) {
    pinfo |= 0x40;
  }

  // Get the SKU ID bit[5-3]
  if (get_cfg_item_num("platform_board_id", &b_id)) {
    res->cc = CC_UNSPECIFIED_ERROR;
    *res_len = 0x00;
    return;
  } else {
...
static void oem_get_poss_pcie_config(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  res->cc = ipmi_get_poss_pcie_config(
      req->payload_id, req->data, req_len, res->data, res_len);

  return;
}

static void oem_set_imc_version(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static int set_fw_update_ongoing(uint8_t fruid, uint16_t tmout) {
  char key[64] = {0};
  char value[64] = {0};
  struct timespec ts;
...
static void oem_set_fw_update_state(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  *res_len = 0;
  if (req_len == 5) {
    // Implicit FRU_ID
    ret = set_fw_update_ongoing(
        req->payload_id, (req->data[1] << 8 | req->data[0]));
  } else if (req_len == 6) {
    // Explicit FRU_ID
    ret =
        set_fw_update_ongoing(req->data[0], (req->data[1] << 8 | req->data[2]));
  } else {
    res->cc = CC_INVALID_LENGTH;
    return;
  }
...
static void oem_bypass_dev_card(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_get_board_id(
  ...
  res->cc = g_fv->get_board_id(
      req->payload_id, req->data, req_len, res->data, res_len);

  return;
}

static void oem_get_80port_record(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  g_fv->get_80port_record(req->payload_id, res->data, 256, (size_t*)res_len);
}

static void oem_get_fw_info(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  if (get_cfg_item_num("fw_info", &fw_info)) {
    res->cc = CC_UNSPECIFIED_ERROR;
    *res_len = 0x00;
    return;
  }
...
static void oem_set_machine_config_info(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_set_flash_info(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  *res_len = 0x0;
  ...
  if (g_fv->get_fru_name(req->payload_id, fruname)) {
    res->cc = CC_UNSPECIFIED_ERROR;
    return;
  }
...
static void oem_get_flash_info(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_get_80port_dword_record(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_get_dev_card_sensor(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
//
static void oem_bbv_power_cycle(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_stor_add_string_sel(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_set_bios_cap_fw_ver(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_set_slot_power_policy(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  *data++ = 0x07; // Power restore policy support(bitfield)
  ...
  res->cc = ipmi_set_slot_power_policy(req->data, res->data);
  if (res->cc == CC_SUCCESS) {
    *res_len = data - &res->data[0];
  }
}

static void oem_get_usb_cdc_status(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_control_usb_cdc(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  if (status_ctrl == DISABLE_USB_CDC) {
    ...
    if (ret == 0) {
      ...
      if (WIFEXITED(ret) == 0) {
        syslog(
            LOG_ERR,
            "%s: failed to disable ether-over-usb (usb0), ret: %d\n",
            __func__,
            ret);
        res->cc = CC_UNSPECIFIED_ERROR;
      }
      ...
      syslog(
          LOG_ERR,
          "%s: failed to disable USB serial console, ret: %d\n",
          __func__,
          ret);
      res->cc = CC_UNSPECIFIED_ERROR;
    }
  } else if (status_ctrl == ENABLE_USB_CDC) {
    // Enable USB serial console and reload USB CDC driver by usbcons.sh, then
    // link up usb0 The 'restart' action will kill all usbmod.sh processes then
    // start usbmon.sh
    ...
    if (ret == 0) {
      ...
      if (WIFEXITED(ret) == 0) {
        syslog(
            LOG_ERR,
            "%s: failed to link up ether-over-usb (usb0), ret: %d\n",
            __func__,
            ret);
        res->cc = CC_UNSPECIFIED_ERROR;
      }
      ...
      syslog(
          LOG_ERR,
          "%s: failed to enable ether-over-usb (usb0), ret: %d\n",
          __func__,
          ret);
      res->cc = CC_UNSPECIFIED_ERROR;
    }
...
static void oem_set_ioc_fw_recovery(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_get_ioc_fw_recovery(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_setup_exp_uart_bridging(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_teardown_exp_uart_bridging(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_set_pcie_info(
  ...
  sprintf(
      key,
      "sys_config/fru%d_pcie_i%02X_s%02X_info",
      req->payload_id,
      req->data[0],
      req->data[1]);
...
static void oem_bios_extra_setup(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void ipmi_handle_oem_storage(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void ipmi_handle(
  ...
  if (gLogEnable) {
    ...
    syslog(
        LOG_INFO,
        "%lu ipmi_handle  netfn %x cmd %x len %d\n",
  ...
  *(unsigned short*)res_len = 0;
  ...
  switch (netfn) {
    ...
    case NETFN_OEM_USB_DBG_REQ:
      syslog(LOG_ERR, "NETFN_OEM_USB_DBG_REQ is not supported in ipmi-v2");
...
static void oem_1s_handle_ipmb_kcs(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  // Remove OEM IPMI Header (including 1 byte for interface type, 3 bytes for
  // IANA ID) The offset moves by one due to the payload ID
...
static void oem_1s_handle_ipmb_req(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
  ...
  switch (req->data[3]) {
    case BIC_INTF_ME:
      ...
      syslog(
          LOG_INFO,
          "oem_1s_handle_ipmb_req: Command received from ME for "
          "payload#%d\n",
          req->payload_id);
#endif
    ...
    case BIC_INTF_SOL:
      ...
      syslog(
          LOG_INFO,
          "oem_1s_handle_ipmb_req: Command received from SOL for "
          "payload#%d\n",
          req->payload_id);
#endif
    ...
    default:
      ...
      syslog(
          LOG_WARNING,
          "oem_1s_handle_ipmb_req: Command received on intf#%d "
          "for payload#%d",
          req->data[3],
          req->payload_id);
      memcpy(res->data, req->data, 4); // IANA ID + Interface type
...
void ipmi_handle_oem_1s(
  ...
  switch (cmd) {
    ...
    case CMD_OEM_1S_INTR:
      ...
      syslog(
          LOG_INFO,
          "ipmi_handle_oem_1s (cmd 0x%02x): 1S server interrupt#%d received "
          "for payload#%d\n",
          cmd,
          req->data[3],
          req->payload_id);
    ...
    case CMD_OEM_1S_POST_BUF:
      ...
      for (i = SIZE_IANA_ID + 1; i <= req->data[SIZE_IANA_ID] + SIZE_IANA_ID;
           i++) {
        g_fv->post_handle(req->payload_id, req->data[i]);
      }
    ...
    case CMD_OEM_1S_PLAT_DISC:
      syslog(
          LOG_INFO,
          "ipmi_handle_oem_1s: Platform Discovery received for "
          "payload#%d\n",
          req->payload_id);
      res->cc = CC_SUCCESS;
      memcpy(res->data, req->data, SIZE_IANA_ID); // IANA ID
    ...
    case CMD_OEM_1S_BIC_RESET:
      syslog(
          LOG_INFO,
          "ipmi_handle_oem_1s: BIC Reset received "
          "for payload#%d\n",
          req->payload_id);
    ...
    case CMD_OEM_1S_BIC_UPDATE_MODE:
      ...
      syslog(
          LOG_INFO,
          "ipmi_handle_oem_1s: BIC Update Mode received "
          "for payload#%d\n",
          req->payload_id);
#endif
      if (req->data[3] == 0x1) {
      ...
      // TODO: remove it.  g_fv->inform_bic_mode(req->payload_id, req->data[3]);
    ...
    case CMD_OEM_1S_GET_SYS_FW_VER:
      if (req_len == 8) { // payload_id, netfn, cmd, IANA[3], data[0] (fru),
                          // data[1] (component)
        memcpy(res->data, req->data, SIZE_IANA_ID); // IANA ID
        res->cc = CC_NOT_SUPP_IN_CURR_STATE;
    ...
    case CMD_OEM_1S_DEV_POWER:
      // payload_id, netfn, cmd, data[0] (device id), data[1] (action), data[2]
      // (data)
      res->cc = CC_NOT_SUPP_IN_CURR_STATE;
      break;
...
static void oem_zion_get_system_mode(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_zion_set_system_mode(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_zion_set_usb_path(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void oem_zion_get_sensor_value(
    unsigned char* request,
    unsigned char req_len,
    unsigned char* response,
    unsigned char* res_len) {
...
static void set_def_restart_cause(uint8_t slot) {
  uint8_t cause;
  if (get_restart_cause(slot, &cause)) {
    ...
    set_restart_cause(slot, RESTART_CAUSE_PWR_ON_PUSH_BUTTON);
  }
}

int conn_handler(client_t* cli) {
  ...
  ipmi_handle(
      req_buf, (unsigned char)req_len, res_buf, (unsigned char*)&res_len);
...
void* wdt_timer(void* arg) {
  ...
  while (1) {
    ...
    if (wdt->valid && wdt->run) {
      // Check if power off
      ret = ipmi_get_server_power(wdt->slot, &status);
    ...
    // Execute actin out of mutex
    if (action) {
      if (ipmi_is_crashdump_ongoing(wdt->slot)) {
        syslog(
            LOG_WARNING,
            "ipmid: fru%u crashdump is ongoing, ignore wdt action %s",
            wdt->slot,
            wdt_action_name[action & 0x7]);
        action = 0;
        continue;
      ...
      set_restart_cause(wdt->slot, RESTART_CAUSE_WATCHDOG_EXPIRATION);
      switch (action) {
        case 1: // Hard Reset
          g_fv->set_server_power(wdt->slot, SERVER_POWER_RESET);
          break;
        case 2: // Power Down
          g_fv->set_server_power(wdt->slot, SERVER_POWER_OFF);
          break;
        case 3: // Power Cycle
          g_fv->set_server_power(wdt->slot, SERVER_POWER_CYCLE);
          break;
        case 0: // no action
        default:
          break;
...
int main(int argc, char** argv) {
  ...
  syslog(LOG_NOTICE, "ipmid: call init_func_vector()\n");
  init_func_vector();

  if (plat_config_parse()) {
    syslog(LOG_ERR, "ipmid: failed parsing the config.json file!");
    exit(1);
  }
  ...
  IPMI_PTHREAD_MUTEX_INIT(m_chassis);
  IPMI_PTHREAD_MUTEX_INIT(m_sensor);
  IPMI_PTHREAD_MUTEX_INIT(m_app);
  IPMI_PTHREAD_MUTEX_INIT(m_storage);
  IPMI_PTHREAD_MUTEX_INIT(m_transport);
  IPMI_PTHREAD_MUTEX_INIT(m_oem);
  IPMI_PTHREAD_MUTEX_INIT(m_oem_1s);
  IPMI_PTHREAD_MUTEX_INIT(m_oem_usb_dbg);
  IPMI_PTHREAD_MUTEX_INIT(m_oem_q);
  IPMI_PTHREAD_MUTEX_INIT(m_oem_zion);

  fru = 1;
  if (get_cfg_item_num("max_slot_num", &slot_num)) {
    syslog(LOG_ERR, "failed to read slot_num\n");
  }
  while (fru <= slot_num) {
  ...
  if (ipc_start_svc(SOCK_PATH_IPMI, conn_handler, MAX_REQUESTS, NULL, &tid) ==
      0) {
    pthread_join(tid, NULL);
  }

  IPMI_PTHREAD_MUTEX_DESTROY(m_chassis);
  IPMI_PTHREAD_MUTEX_DESTROY(m_sensor);
  IPMI_PTHREAD_MUTEX_DESTROY(m_app);
  IPMI_PTHREAD_MUTEX_DESTROY(m_storage);
  IPMI_PTHREAD_MUTEX_DESTROY(m_transport);
  IPMI_PTHREAD_MUTEX_DESTROY(m_oem);
  IPMI_PTHREAD_MUTEX_DESTROY(m_oem_1s);
  IPMI_PTHREAD_MUTEX_DESTROY(m_oem_usb_dbg);
  IPMI_PTHREAD_MUTEX_DESTROY(m_oem_q);
