...
fby2_common_fru_name(uint8_t fru, char *str) {
  ...
  switch(fru) {
    ...
    //for aggregate sensor
    case 0xFF:
      return -1;

    default:
      syslog(LOG_WARNING, "%s: Wrong fru id - 0x%x", __func__, fru);
      return -1;
  }
...
fby2_common_fru_id(char *str, uint8_t *fru) {
  ...
  if (!strcmp(str, "all")) {
    ...
    *fru = FRU_BMC;
  } else if (!strcmp(str, "aggregate")) {
    return -1;
  } else {
    syslog(LOG_WARNING, "fby2_common_fru_id: Wrong fru name: %s\n", str);
    return -1;
  }
