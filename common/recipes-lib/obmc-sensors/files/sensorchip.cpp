...
unique_ptr<Sensor> SensorChip::make_sensor(const sensors_chip_name *chip, const sensors_feature *feature)
{
  return unique_ptr<Sensor>(new Sensor(chip, feature));
}
...
void SensorChip::enumerate()
{
  for (int nf = 0; ;) {
    ...
    unique_ptr<Sensor> snr = make_sensor(chip, feature);
    addsensor(move(snr));
  }
}
