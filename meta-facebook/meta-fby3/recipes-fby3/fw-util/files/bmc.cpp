    ...
    if (dual_flash) {
      ...
      if (vboot != VBOOT_NO_SUPPORT)  {
        ...
        if (vboot == VBOOT_HW_ENFORCE) {
          ...
          static BmcComponent bmc("bmc", "bmc", "bmc", system, "flash1rw", "u-boot", BMC_RW_OFFSET, ROMX_SIZE);
          // Locked down, Allow getting version of ROM, but do not allow
          // upgrading it.
          static BmcComponent rom("bmc", "bmc", "rom", system, "", "u-bootro");
        } else {
          // Unlocked Flash, Upgrade the full BMC flash1.
          static BmcComponent bmc("bmc", "bmc", "bmc", system, "flash1", "u-boot");
          // Unlocked flash, Allow upgrading the ROM.
          static BmcComponent rom("bmc", "bmc", "rom", system, "flash0", "u-bootro");
        }
      } else {
        // non-verified-boot. BMC boots off of flash0.
        static BmcComponent bmc("bmc", "bmc", "bmc", system, "flash0", "u-boot");
        // Dual flash supported, but not in verified boot format.
        // Allow flashing the second flash. Read version from u-boot partition.
        static BmcComponent bmcalt("bmc", "bmc", "altbmc", system, "flash1", "u-bootx");
      }
      // Verified boot supported and in dual-flash mode.
    } else {
      // We just have the one flash. Allow upgrading flash0.
      static BmcComponent bmc("bmc", "bmc", "bmc", system, "flash0");
    }
  }
};
