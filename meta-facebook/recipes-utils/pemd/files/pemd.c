 ...
 */
static
inline int check_threshold_sensor_status(struct threshold_sensor_t *sensor) {
       ...
       (sensor->value <= sensor->threshold[SENSOR_LCR_THRESH])) {
        ...
        if((sensor->threshold[SENSOR_LNC_THRESH] != 0) &&
           (sensor->value <= sensor->threshold[SENSOR_LNC_THRESH])) {
            ...
            if (sensor->threshold_cnt[SENSOR_LNC_THRESH] >=
                                                         PEM_CHECK_RETRY_CNT) {
...
static void init_pem_sensors(uint8_t pem_num, struct pem_t *pem) {
    for(int i = 0; i < PEM_THRESHOLD_NUM; i++) {
        pem->sensors[i].index = pem_num * PEM_SENSOR_NUM +
                     PEM1_SENSOR_IN_VOLT + i;
        memset(pem->sensors[i].sensor_name, 0,
                    sizeof(pem->sensors[i].sensor_name)/sizeof(char));
        pal_get_sensor_name(pem_num + FRU_PEM1, pem->sensors[i].index,
    ...
    for(int i = 0; i < PEM_DISCRETE_NUM; i++) {
        int normal;
        pem->discrete[i].index = pem_num * PEM_SENSOR_NUM +
                                         PEM1_SENSOR_FAULT_OV + i;
        memset(pem->discrete[i].sensor_name, 0,
                         sizeof(pem->discrete[i].sensor_name)/sizeof(char));
        pal_get_sensor_name(pem_num + FRU_PEM1, pem->discrete[i].index,
                                             pem->discrete[i].sensor_name);
...
static int dump_pem_sensors(uint8_t pem_num, struct pem_t *pem) {
    ...
    for (int i = 0; i < PEM_DISCRETE_NUM; i++) {
        ...
        if (pal_sensor_discrete_read_raw(pem_num + FRU_PEM1,
                         pem->discrete[i].index, &pem->discrete[i].value)) {
            return -1;
        }
    }
    return 0;
    ...
    for (int i = 0; i < PEM_DISCRETE_NUM; i++) {
        ...
        case PEM4_SENSOR_STATUS_FET_SHORT:
            ...
            if(check_bool_fail(&pem->discrete[i])) {
                OBMC_CRIT("PEM %d: %s occurring", pem_num + 1,
                                     pem->discrete[i].sensor_name);
            }
            break;
        default:
...
static void pem_handler(uint8_t pem_num, uint8_t sensors_status) {
    ...
    if(sensors_status & PEM_HANDLER_SHUTDOWN) {
        OBMC_CRIT("PEM %d do COMe/TH3/GB shutdown", pem_num + 1);
        if (pal_set_server_power(0, SERVER_POWER_OFF)) {
            OBMC_CRIT("PEM %d do COMe shutdown failed", pem_num + 1);
        }
        if (pal_set_thx_power_off()) {
            OBMC_CRIT("PEM %d do THx/GB shutdown failed", pem_num + 1);
        }
    }
}

/*
 * Starts monitoring all the sensors on a pem for all the
 * threshold/discrete values.
    ...
    while(1) {
        ...
        if (pem_status != 1) {
            ...
            if(pem_retry >= PEM_CHECK_RETRY_CNT) {
                OBMC_CRIT("PEM %d monitor exit because PEM is not ready",
                                 pem_num + 1);
                return NULL;
            }
            sleep(5);
        ...
        pem_handler(pem_num, sensors_status);
    }
}
...
run_pemd(void) {
    ...
    for(int i = 0; i < PEM_NUM; i++) {
        if(pthread_create(&thread_pem[i], NULL, pem_monitor,
                                         (void*)(uintptr_t)i) < 0) {
