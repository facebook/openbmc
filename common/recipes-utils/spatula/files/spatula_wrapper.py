...
class SpatulaWrapper(object):
    def __init__(
        self, address="fe80::2", interface="usb0", port=8087, tls=False, lldp=False
    ):
        ...
        self.url = "{proto}://{address}%{iface}:{port}".format(
            proto=self.proto, address=address, iface=self.interface, port=self.port
        )
    ...
    def _getLldpMessage(self):
        try:
            ...
            raise Exception(
                "failed discovering management system via LLDP: {}".format(err)
            )
    ...
    def _getSpatula(self, endpoint="/spatula"):
        ...
        if self.lldp:
            for lldp in self._getLldpMessage():
                urls.append(
                    "{proto}://{address}%{iface}:{port}{endpoint}".format(
                        proto=self.proto,
    ...
    def _execute(self):
        try:
            ...
            spatula = subprocess.Popen(
                [SPATULA_FILE], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
...
if __name__ == "__main__":
    ...
    wrapper = SpatulaWrapper(
        interface=params.interface, tls=params.tls, port=params.port, lldp=params.lldp
    )
