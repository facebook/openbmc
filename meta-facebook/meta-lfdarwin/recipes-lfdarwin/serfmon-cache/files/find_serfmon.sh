...
# Default-Start:     S
# Default-Stop:
# Short-Description:  Find Serfmon string obmc-console log and cache it
### END INIT INFO
...
CHASSIS_EEPROM_CACHE="/mnt/data/.chassis_eeprom"
META_EEPROM_CACHE="/mnt/data/.chassis_meta_eeprom"
...
maybe_update_cache() {
    # Check the obmc-console logfile for the given regex. If found, update the cache_file
    ...
    if grep -qE "$regex" "$OBMC_LOGFILE"; then
        # Detect if string found in obmc-console log
        ...
        # If obmc-console's string value doesn't match the cache, update it
