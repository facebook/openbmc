...
int main(int argc, char **argv) {
    ...
    if (strcmp("rackmoninfo", callname) == 0) {
      cmd.type = COMMAND_TYPE_DUMP_DATA_INFO;
    }
    ...
    if (argc > 1 && (strcmp("info", argv[1]) == 0)) {
      cmd.type = COMMAND_TYPE_DUMP_DATA_INFO;
    }
    ...
    if(cmd.type == 0) {
      fprintf(stderr, "Usage: %s { status | data | info | force_scan | pause | resume }\n", callname);
      exit(1);
    }
