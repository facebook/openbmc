...
int main(int argc, char **argv)
{
    ...
    update->add_option("-c,--chip", chip, "LCMXO3LF-4300|LCMXO3LF-6900|LCMXO3D-4300|LCMXO3D-9400")->required();

    update->add_flag("-v,--verbose", debugMode, "debug mode");

    auto version = app.add_subcommand("version", "Get Frimware Version");

    version->add_option("-i,--interface", interface, "interface")
    ->required();
    ...
    auto cpldManager = CpldLatticeManager(bus, addr, imagePath, chip, interface, debugMode);
