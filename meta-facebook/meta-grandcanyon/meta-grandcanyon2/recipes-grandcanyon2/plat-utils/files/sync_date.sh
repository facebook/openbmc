...
##### Check if time is already valid (e.g., synced by NTP one-shot)
current_year=$(date +%Y)
if [ "$current_year" -ge "$THRESHOLD_YEAR" ]; then
  logger -s -p user.info -t sync-date "BMC time already valid (year=$current_year), skipping sync"
