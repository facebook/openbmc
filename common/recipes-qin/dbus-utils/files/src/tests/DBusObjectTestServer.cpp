...
int main (int argc, char* argv[]) {
  ...
  std::shared_ptr<DBus> sDBus(new DBus("org.openbmc.Chassis", &interface));
  DBus &dbus = *(sDBus.get());
  GMainLoop *loop;
  std::thread t;
  ...
  // the interface is inserted as default interface
  ...
  // the above objects can be pinged through DBusObjectInterface
