...
def logDump():
    ...
    file_list = [file for file in file_list if re.search("mTerm_wedge.log.*", file)]
    for file in file_list:
        print("{}".format(runCmd("cat /var/log/" + file, echo=True)))


def dumpNetworkStatus():
    print("##############################################")
    print("########## Network interface status ##########")
    print("##############################################\n")
    print(runCmd("ifconfig -a", echo=True))
    print(runCmd("ip address", echo=True))
...
def i2cDetectDump():
    ...
    for bus in [0, 1, 3, 4, 5, 6, 11, 12, 13, 14, 15]:
        print(
            "##### SMBus{} INFO #####\n{}".format(
                bus,
                runCmd(
...
def sensorDump():
    print(
        "##### SENSOR DUMP #####\n{}".format(runCmd("sensors", verbose=True, echo=True))
    )
...
def showtech(verboseLevel=0):
    ...
    print(
        "##### Aspeed Chip Revision #####\n{}".format(
            runCmd("/usr/local/bin/aspeed_chip_rev.sh")
        )
    )
    print(
        "##### BMC Board Revision #####\n{}".format(
