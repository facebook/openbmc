...
class CommonFanUtilBasedFansTest(BaseFansTest):
        ...
        for line in info:
            ...
            if not m:
                if re.match(r'^\s*$', line):
                    continue
                attrs = line.split(": ")
                self.assertIn(attrs[0], ["Fan Mode", "Sensor Fail", "Fan Fail"], "Accepted printed attributes")
    ...
    def fan_read(self, fan=None):
        ...
        data = self.get_fan_speed(fan_id=fan)
        Logger.info("Fan dump:\n" + data)
        self.assertTrue(self.verify_get_fan_speed_output(data, fan),
            "Get fan speed failed")
    ...
    def fan_set_and_read(self, fan_id=None, pwm=None):
        ...
        self.assertAlmostEqual(speed[0], pwm, delta=5,
            msg="Speed was not set to {} pwm [FAILED]".format(pwm))
    ...
    def test_set_all_fans_and_read(self):
        ...
        for s in speed:
            self.assertAlmostEqual(s, pwm, delta=5,
                msg="Speed was not set to {} pwm [FAILED]".format(pwm))
