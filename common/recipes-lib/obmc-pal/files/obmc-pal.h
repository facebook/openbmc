...
// lpc-pcc postcode
#define PCC_PORT1      (0x40)
#define PCC_PORT2      (0x41)
#define PCC_PORT3      (0x42)
#define PCC_PORT4      (0x43)
#define PCC_FIFO_SIZE  (257)  // 256 postcodes avaliable
...
int pal_get_lpc_pcc_record(uint8_t slot, uint8_t *buf, size_t max_len, size_t *len);
...
int pal_mrc_warning_detect(uint8_t slot, uint32_t postcode);
...
int pal_ignore_sel(uint8_t fru, uint8_t *sel);
...
int pal_handle_arm_sbmr(uint8_t fru, uint8_t *tbuf, uint8_t tlen, uint8_t *rbuf, uint8_t *rlen);
...
int pal_check_psb_error(uint32_t postcode);
