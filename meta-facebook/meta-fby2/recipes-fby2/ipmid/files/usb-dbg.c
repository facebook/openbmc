...
{
  {"SOC_TEMP:"    , BIC_SENSOR_SOC_TEMP        , "C"   , 0},
  {"HSC_PWR:"     , SP_SENSOR_HSC_IN_POWER     , "W"   , 5},
  {"HSC_VOL:"     , SP_SENSOR_HSC_IN_VOLT      , "V"   , 5},
  {"FAN0:"        , SP_SENSOR_FAN0_TACH        , "RPM" , 5},
  {"FAN1:"        , SP_SENSOR_FAN1_TACH        , "RPM" , 5},
  {"SP_INLET:"    , SP_SENSOR_INLET_TEMP       , "C"   , 5},
  {"SOC_VR_TEMP:" , BIC_SENSOR_VCCIN_VR_TEMP   , "C"   , 0},
  {"SOC_VR_PWR:"  , BIC_SENSOR_VCCIN_VR_POUT   , "W"   , 0},
  {"DIMMA0_TEMP:" , BIC_SENSOR_SOC_DIMMA0_TEMP , "C"   , 0},
  {"DIMMA1_TEMP:" , BIC_SENSOR_SOC_DIMMA1_TEMP , "C"   , 0},
  {"DIMMB0_TEMP:" , BIC_SENSOR_SOC_DIMMB0_TEMP , "C"   , 0},
  {"DIMMB1_TEMP:" , BIC_SENSOR_SOC_DIMMB1_TEMP , "C"   , 0},
  {"DIMMD0_TEMP:" , BIC_SENSOR_SOC_DIMMD0_TEMP , "C"   , 0},
  {"DIMMD1_TEMP:" , BIC_SENSOR_SOC_DIMME1_TEMP , "C"   , 0},
  {"DIMME0_TEMP:" , BIC_SENSOR_SOC_DIMMD0_TEMP , "C"   , 0},
  {"DIMME1_TEMP:" , BIC_SENSOR_SOC_DIMME1_TEMP , "C"   , 0},
};
...
static int
get_syscfg_text(uint8_t slot, char *text) {
...
plat_chk_cri_sel_update(uint8_t *cri_sel_up) {
  ...
  uint8_t pos;
  static uint8_t pre_pos = 0x0F;

  if (pal_get_hand_sw(&pos) == 0) {
    if (pre_pos != pos) {
      pre_pos = pos;
      *cri_sel_up = 1;
      return 0;
    ...
    if ((stat("/mnt/data/cri_sel", &file_stat) == 0) && 
      ...
      while (fgets(line_buff, 256, fp)) {
        ...
        if (strlen(ptr) > 2) {
          ...
          // Write new message
          frame_sel.insert(&frame_sel, ptr+2, 0);
        }
      }
    }
    fclose(fp);
...
plat_udbg_get_cri_sensor(uint8_t frame, uint8_t page, uint8_t *next, uint8_t *count, uint8_t *buffer) {
  ...
  if (page == 1) {
    ...
    for (i = 0; i < sensor_count; i++) {
      if ((pos == HAND_SW_BMC) && !cri_sensor[i].fru)
        continue;

      temp_thresh[0] = 0;
      fru = (cri_sensor[i].fru) ? cri_sensor[i].fru : pos;
      ret = sensor_cache_read(fru, cri_sensor[i].sensor_num, &fvalue);
      if (ret) {
        strcpy(temp_val, "NA");
      } else {
        if (sdr_get_snr_thresh(fru, cri_sensor[i].sensor_num, &thresh) == 0) {
          if (fvalue >= thresh.ucr_thresh)
            strcpy(temp_thresh, "/UCT");
          else if (fvalue <= thresh.lcr_thresh)
            strcpy(temp_thresh, "/LCT");
        }
...
plat_udbg_get_info_page(uint8_t frame, uint8_t page, uint8_t *next, uint8_t *count, uint8_t *buffer) {
  ...
  if (page == 1) {
    ...
    if (memcmp(lan_config.ip_addr, zero_ip6_addr, SIZE_IP_ADDR)) {
    ...
    // BIOS ver
    if ((pos != HAND_SW_BMC) && !pal_get_sysfw_ver(pos, (uint8_t *)line_buff)) {
      ...
      ret = bic_me_xmit(pos, (uint8_t *)line_buff, 2, (uint8_t *)line_buff, &rlen);
      if (!ret && !line_buff[0]) {
        strcpy(line_buff, (line_buff[3] & 0x80)?"recovery mode":"operation mode");
    ...
    // Board ID
    sprintf(path, "/sys/class/gpio/gpio%d/value", GPIO_BOARD_ID);
    if (!read_device(path, &boardid)) {
      sprintf(line_buff, "%02d", boardid);
...
static uint8_t panel_boot_order(uint8_t item) {
  uint8_t buff[MAX_VALUE_LEN], pickup, len, pos, i;

  if (!pal_get_hand_sw(&pos) && (pos != HAND_SW_BMC) && !pal_get_boot_order(pos, buff, buff, &len)) {
    ...
    // remove empty items
    for (i--; !panels[PANEL_BOOT_ORDER].item_str[i][0] && (i > 0); i--);
...
static uint8_t panel_power_policy(uint8_t item) {
  ...
  if (!pal_get_hand_sw(&pos) && (pos != HAND_SW_BMC)) {
    sprintf(key, "slot%d_por_cfg", pos);

    switch (item) {
      case 1:
        pal_set_key_value(key, "on");
        break;
      case 2:
        pal_set_key_value(key, "lps");
        break;
      case 3:
        pal_set_key_value(key, "off");
        break;
    }
