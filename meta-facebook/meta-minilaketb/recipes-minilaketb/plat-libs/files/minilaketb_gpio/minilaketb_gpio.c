...
int
minilaketb_get_gpio_name(uint8_t fru, uint8_t gpio, char *name) {

  //TODO: Add support for BMC GPIO pins
  if (fru < 1 || fru > 4) {
#ifdef DEBUG
    syslog(LOG_WARNING, "minilaketb_get_gpio_name: Wrong fru %u", fru);
#endif
    return -1;
  }
...
#ifdef DEBUG
    syslog(LOG_WARNING, "minilaketb_get_gpio_name: Wrong gpio pin %u", gpio);
#endif
    return -1;
  }
