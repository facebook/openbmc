...
do_install:append() {
    ...
    install -m 0644 ${UNPACKDIR}/configure-eth0.path ${D}${systemd_system_unitdir}
}

FILES:${PN}:append = " \
    ${systemd_system_unitdir}/eth0_mac_fixup.service \
    ${systemd_system_unitdir}/configure-eth0.service \
    ${systemd_system_unitdir}/configure-eth0.path \
"

SYSTEMD_SERVICE:${PN}:append:fb-fboss = "eth0_mac_fixup.service configure-eth0.service configure-eth0.path"
