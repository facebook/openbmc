...
def populate_server_node(r_api, num, is_read_only=True):
    ...
    r_force_boot_bios_setup = tree("force_boot_bios_setup", data = get_node_bios_force_boot_setup("slot" + repr(num), is_read_only))
    r_boot_order = tree("boot_order", data = get_node_bios_boot_order("slot" + repr(num), is_read_only))
...
def init_plat_tree(is_read_only=True):
    ...
    r_spb = tree("spb", data = get_node_spb(is_read_only))
    ...
    r_temp = tree("logs", data = get_node_logs("spb", is_read_only))
    r_spb.addChild(r_temp)
