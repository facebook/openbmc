...
int
netlakenext_common_me_ipmb_wrapper(uint8_t netfn, uint8_t cmd,
...
int
netlakenext_common_me_get_fw_ver(ipmi_dev_id_t *dev_id) {
  int ret;
  uint8_t rlen = 0;
  ...
  ret = netlakenext_common_me_ipmb_wrapper(NETFN_APP_REQ, CMD_APP_GET_DEVICE_ID, NULL, 0, (uint8_t *)dev_id, &rlen);
  return ret;
}
