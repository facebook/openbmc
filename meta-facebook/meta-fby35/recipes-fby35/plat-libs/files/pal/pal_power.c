...
server_power_12v_on(uint8_t fru) {
  ...
  snprintf(cmd, sizeof(cmd), "sv start ipmbd_%d > /dev/null 2>&1", fby35_common_get_bus_id(fru));
  if (system(cmd) != 0) {
...
server_power_12v_off(uint8_t fru) {
  ...
  snprintf(cmd, sizeof(cmd), "sv stop ipmbd_%d > /dev/null 2>&1", fby35_common_get_bus_id(fru));
  if (system(cmd) != 0) {
      syslog(LOG_WARNING, "[%s] %s failed\n", __func__, cmd);
      ret = PAL_ENOTSUP;
      goto error_exit;
