...
int
minilaketb_common_fru_name(uint8_t fru, char *str) {
...
int
minilaketb_common_fru_id(char *str, uint8_t *fru) {
...
generate_dump(void *arg) {
  ...
  minilaketb_common_fru_name(fru, fruname);
...
int
minilaketb_common_crashdump(uint8_t fru) {
  ...
  if (t_dump[fru-1].is_running) {
    ...
    if (ret == ESRCH) {
      syslog(LOG_INFO, "minilaketb_common_crashdump: No Crashdump pthread exists");
    } else {
      ...
      system(cmd);
#ifdef DEBUG
      syslog(LOG_INFO, "minilaketb_common_crashdump: Previous crashdump thread is cancelled");
#endif
    }
  }
