...
class FscdSysfsOperationalTester(BaseFscdUnitTest):
    ...
    def test_fscd_operation(self):
        ...
        self.assertEqual(int(pwm_value), 12,
                         'Incorrect pwm value was set expected=12 computed=%d'
                         % int(pwm_value))
        ...
        pwm_cmd = 'cat ./test-data/test-sysfs-data/fantray1_pwm'
        pwm_value = Popen(pwm_cmd, shell=True, stdout=PIPE).stdout.read()
        self.assertEqual(int(pwm_value), 20,
                         'Incorrect pwm value was set expected=20 computed=%d'
                         % int(pwm_value))
        ...
        pwm_cmd = 'cat ./test-data/test-sysfs-data/fantray1_pwm'
        pwm_value = Popen(pwm_cmd, shell=True, stdout=PIPE).stdout.read()
        self.assertEqual(int(pwm_value), 28,
                         'Incorrect pwm value was set expected=28 computed=%d'
                         % int(pwm_value))
