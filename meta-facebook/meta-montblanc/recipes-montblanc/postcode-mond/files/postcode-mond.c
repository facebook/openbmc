...
#define PWR_FORCE_OFF    "/sys/bus/i2c/devices/12-0060/pwr_force_off"
#define PWR_COME_EN    "/sys/bus/i2c/devices/12-0060/pwr_come_en"
...
static volatile int fd_post = -1, fd_curr_file = -1, fd_pwr_off = -1, fd_pwr_en = -1;

/* Init file handles */
int init_all_fds(const char *snoop_dev) {
    fd_pwr_off = open(PWR_FORCE_OFF, O_RDONLY);
    fd_pwr_en = open(PWR_COME_EN, O_RDONLY);
    fd_post = open(snoop_dev, O_RDONLY | O_NONBLOCK);
    fd_curr_file = open(PATH_POSTCODE_CURRENT, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd_post < 0 || fd_curr_file < 0 || fd_pwr_off < 0 || fd_pwr_en < 0) {
...
void cleanup(int signum) {
    ...
    if (fd_pwr_off >= 0) close(fd_pwr_off);
    if (fd_pwr_en >= 0) close(fd_pwr_en);
    closelog();
    exit(0);
}
...
int is_power_on() {
    ...
    lseek(fd_pwr_off, 0, SEEK_SET);
    lseek(fd_pwr_en, 0, SEEK_SET);
    read(fd_pwr_off, buf5, 15);
    read(fd_pwr_en, buf12, 15);
