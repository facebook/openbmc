...
def _fork_procs_for_eeproms(card):
    procs = []
    api_fruid = 'http://[{}]:8080/api/sys/mb/fruid'.format(
        card.link_local_addr())
...
def _get_serial_and_location(card, proc):
    ...
    return SnAndLocation(serial=eeprom['Product Serial Number'].lower(),
...
def _get_serials_and_loc_for_card(card, procs):
    ...
    scm_serial_and_loc = (_get_serial_and_location(card, procs[1]) if card.has_scm
                          else None)
...
def get_all_serials_and_locations():
    ...
    for card, procs in card_to_procs.items():
        try:
            ...
            syslog.syslog(syslog.LOG_ERR, 'Error getting SN, Location '
                          'from {} error: {}'.format(card, e))
            continue
        ...
        for idx, slot_type in enumerate(['bmc', 'scm']):
            d = json.dumps(_populate_serial_and_loc(slot_type,
                                                    serial_and_locs[idx]))
