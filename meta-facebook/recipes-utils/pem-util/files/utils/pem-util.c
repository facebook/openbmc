...
static void
print_usage(const char *name, const char pem_num) {
  int i;
  printf("Usage: %s <", name);
  for (i = 0; i < pem_num; i++)
  {
...
char get_pem_id(const char *name, const char pem_num)
{
  char ret = -1;
  char i = 0;
  char buffer[32];
  for (i = 0; i < pem_num; i++)
  {
    snprintf(buffer, sizeof(buffer), "pem%d", i+1);
...
main(int argc, const char *argv[]) {
  ...
  if (argc < 3) {
    print_usage(argv[0], PEM_NUM);
    return -1;
  }

  pem_num = get_pem_id(argv[1], PEM_NUM);
  if (pem_num < 0) {
    print_usage(argv[0], PEM_NUM);
    return -1;
  }
